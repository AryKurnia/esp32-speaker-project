// ================================================================
// ESP32 Bluetooth Speaker with DSP Crossover
// Author: Ary
// ================================================================

// ── SECTION 1: INCLUDES & DEFINES ───────────────────────────────
#include <Arduino.h>
#include "LittleFS.h"
#include "BluetoothA2DPSink.h"
#include "driver/i2s.h"
#include <Preferences.h>
#include <U8g2lib.h>
#include <Wire.h>

// ── DSP FUNCTIONS (implementasi manual, tanpa library) ────────
void dsp_gen_lpf(float *c, float f, float q) {
    float w  = 2.0f * M_PI * f;
    float cs = cosf(w), sn = sinf(w);
    float alpha = sn / (2.0f * q);
    float a0    = 1.0f + alpha;
    c[0] =  ((1.0f - cs) / 2.0f) / a0; // b0
    c[1] =   (1.0f - cs)          / a0; // b1
    c[2] =  ((1.0f - cs) / 2.0f) / a0; // b2
    c[3] =  (-2.0f * cs)          / a0; // a1
    c[4] =   (1.0f - alpha)       / a0; // a2
}

void dsp_gen_hpf(float *c, float f, float q) {
    float w  = 2.0f * M_PI * f;
    float cs = cosf(w), sn = sinf(w);
    float alpha = sn / (2.0f * q);
    float a0    = 1.0f + alpha;
    c[0] =  ((1.0f + cs) / 2.0f) / a0; // b0
    c[1] = -((1.0f + cs))         / a0; // b1
    c[2] =  ((1.0f + cs) / 2.0f) / a0; // b2
    c[3] =  (-2.0f * cs)          / a0; // a1
    c[4] =   (1.0f - alpha)       / a0; // a2
}

void dsp_biquad(float *in, float *out, int len, float *c, float *w) {
    for (int i = 0; i < len; i++) {
        float d = in[i] - c[3] * w[0] - c[4] * w[1];
        out[i]  = c[0] * d  + c[1] * w[0] + c[2] * w[1];
        w[1]    = w[0];
        w[0]    = d;
    }
}

#include "freertos/FreeRTOS.h"
#include "freertos/ringbuf.h"

// Pin Definitions
#define BTLED   2
#define ENC_CLK 32
#define ENC_DT  33
#define ENC_SW  27  // ← GPIO27 (bukan 34, karena 34 tidak support pull-up)

BluetoothA2DPSink a2dp_sink;

// ── SECTION 2: RING BUFFER ───────────────────────────────────────
#define AUDIO_BUF_SIZE 16384 // 16KB per buffer

RingbufHandle_t ringbuf_bt  = NULL;
RingbufHandle_t ringbuf_wav = NULL;

void init_ring_buffers() {
    ringbuf_bt  = xRingbufferCreate(AUDIO_BUF_SIZE, RINGBUF_TYPE_BYTEBUF);
    ringbuf_wav = xRingbufferCreate(AUDIO_BUF_SIZE, RINGBUF_TYPE_BYTEBUF);

    if (ringbuf_bt == NULL || ringbuf_wav == NULL) {
        Serial.println("[ERROR] Gagal alokasi Ring Buffer!");
    } else {
        Serial.println("[OK] Ring Buffer siap");
    }
}

// ── SECTION 3: I2S CONFIG ────────────────────────────────────────
i2s_pin_config_t pin_config = {
    .mck_io_num   = I2S_PIN_NO_CHANGE,
    .bck_io_num   = 25,
    .ws_io_num    = 26,
    .data_out_num = 22,
    .data_in_num  = I2S_PIN_NO_CHANGE
};

i2s_config_t i2s_config = {
    .mode                 = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_TX),
    .sample_rate          = 44100,
    .bits_per_sample      = I2S_BITS_PER_SAMPLE_16BIT,
    .channel_format       = I2S_CHANNEL_FMT_RIGHT_LEFT,
    .communication_format = (i2s_comm_format_t)(I2S_COMM_FORMAT_STAND_I2S),
    .intr_alloc_flags     = 0,
    .dma_buf_count        = 16,
    .dma_buf_len          = 512,
    .use_apll             = false,
    .tx_desc_auto_clear   = true,
    .fixed_mclk           = 0
};

// ── SECTION 4: DSP CONFIG ────────────────────────────────────────
float crossover_freq    = 5000.0f;
const float SAMPLE_RATE = 44100.0f;
const float Q_LR        = 0.7071f; // Butterworth Q untuk Linkwitz-Riley

// Koefisien biquad [b0, b1, b2, a1, a2]
float coeffs_lpf1[5], coeffs_lpf2[5];
float coeffs_hpf1[5], coeffs_hpf2[5];

// Delay line state — wajib dipisah tiap filter
float w_lpf1[2] = {0}, w_lpf2[2] = {0};
float w_hpf1[2] = {0}, w_hpf2[2] = {0};

// Buffer DSP — global, fixed size
static float buf_mono[1024];
static float buf_lpf[1024],  buf_lpf2[1024];
static float buf_hpf[1024],  buf_hpf2[1024];

// Gain
float gain_woofer  = 1.0f;
float gain_tweeter = 1.0f;

// Mode
bool mono_mode = true; // true = crossover, false = stereo passthrough

// ── SECTION 5: UI STATE MACHINE ──────────────────────────────────
enum UIState {
    IDLE,
    NAVIGATE,
    EDIT
};

enum UIParam {
    PARAM_FREQ,
    PARAM_GAIN_W,
    PARAM_GAIN_T,
    PARAM_WAV_VOL,
    PARAM_MODE,
    PARAM_COUNT
};

enum UIPage {
    PAGE_NOWPLAY,  // default
    PAGE_CONFIG,
    PAGE_CONTROL
};

enum ControlAction {
    CTRL_PREV,
    CTRL_PAUSE,
    CTRL_PLAY,
    CTRL_NEXT,
    CTRL_COUNT
};

// State variables
UIState       ui_state       = IDLE;
UIParam       selected_param = PARAM_FREQ;
UIPage        current_page   = PAGE_NOWPLAY;
ControlAction selected_action = CTRL_PLAY;

// Timeout
unsigned long last_interaction         = 0;
unsigned long last_control_interaction = 0;
const unsigned long TIMEOUT_MS         = 3000; // CONFIG idle timeout
const unsigned long CONTROL_TIMEOUT_MS = 5000; // CONTROL page timeout

// Blink
bool          blink_state = false;
unsigned long last_blink  = 0;
const unsigned long BLINK_FAST = 200; // EDIT
const unsigned long BLINK_SLOW = 600; // NAVIGATE

// ── SECTION 6: TRACK INFO & SCROLL ──────────────────────────────
struct TrackInfo {
    char     title[64];
    char     artist[64];
    uint32_t duration_ms;
    uint32_t position_ms;
    bool     has_data;
};

TrackInfo track_info = {"", "", 0, 0, false};

// Scroll state
int           scroll_x_title  = 0;
int           scroll_x_artist = 0;
unsigned long last_scroll      = 0;
bool          scroll_pausing   = false;
unsigned long scroll_pause_start = 0;

const int SCROLL_SPEED_MS = 150;
const int SCROLL_PAUSE_MS = 2000;

// ── SECTION 7: OLED (U8g2) ───────────────────────────────────────
U8G2_SSD1306_128X32_UNIVISION_F_HW_I2C u8g2(
    U8G2_R0,
    U8X8_PIN_NONE,
    19,  // SCL
    21   // SDA
);

// ── SECTION 8: DSP FUNCTIONS ─────────────────────────────────────
void update_crossover(float freq_hz) {
    float f_norm = freq_hz / SAMPLE_RATE;

    dsp_gen_lpf(coeffs_lpf1, f_norm, Q_LR);
    dsp_gen_lpf(coeffs_lpf2, f_norm, Q_LR);
    dsp_gen_hpf(coeffs_hpf1, f_norm, Q_LR);
    dsp_gen_hpf(coeffs_hpf2, f_norm, Q_LR);

    // Reset delay line
    memset(w_lpf1, 0, sizeof(w_lpf1));
    memset(w_lpf2, 0, sizeof(w_lpf2));
    memset(w_hpf1, 0, sizeof(w_hpf1));
    memset(w_hpf2, 0, sizeof(w_hpf2));

    Serial.printf("[DSP] Crossover updated: %.0f Hz\n", freq_hz);
}

void apply_crossover(int16_t* mixed, int16_t* output, size_t num_frames) {
    // Mono mix
    for (size_t i = 0; i < num_frames; i++) {
        buf_mono[i] = (float)(mixed[i * 2] + mixed[i * 2 + 1]) / 2.0f;
    }

    dsp_biquad(buf_mono, buf_lpf,  num_frames, coeffs_lpf1, w_lpf1);
    dsp_biquad(buf_lpf,  buf_lpf2, num_frames, coeffs_lpf2, w_lpf2);
    dsp_biquad(buf_mono, buf_hpf,  num_frames, coeffs_hpf1, w_hpf1);
    dsp_biquad(buf_hpf,  buf_hpf2, num_frames, coeffs_hpf2, w_hpf2);

    // Output: L → Woofer, R → Tweeter
    for (size_t i = 0; i < num_frames; i++) {
        output[i * 2]     = (int16_t)constrain(buf_hpf2[i] * gain_tweeter, -32768, 32767); // R → Tweeter
        output[i * 2 + 1] = (int16_t)constrain(buf_lpf2[i] * gain_woofer,  -32768, 32767); // L → Woofer
    }
}

// ── SECTION 9: WAV TASK ──────────────────────────────────────────
static char   wav_to_play[32] = "";
float         wav_volume       = 0.7f;
volatile bool is_wav_playing   = false;

void wav_task(void *param) {
    if (strlen(wav_to_play) > 0) {
        File file = LittleFS.open(wav_to_play, "r");
        if (file) {
            file.seek(44); // skip WAV header

            // WAV: 11025Hz mono → upsample 4x → 44100Hz stereo
            const int    RATIO        = 4;
            const size_t READ_SAMPLES = 64; // baca 64 sample per loop

            uint8_t  raw[READ_SAMPLES * 2];       // 128 bytes
            int16_t  stereo[READ_SAMPLES * RATIO * 2]; // 512 int16

            int prefill_count = 0;

            while (file.available()) {
                size_t bytes_read   = file.read(raw, sizeof(raw));
                size_t num_samples  = bytes_read / 2;
                size_t stereo_idx   = 0;

                for (size_t i = 0; i < num_samples; i++) {
                    int16_t sample = (raw[i * 2 + 1] << 8) | raw[i * 2];
                    int16_t s_vol  = (int16_t)(sample * wav_volume);

                    // Upsample 4x + stereo
                    for (int r = 0; r < RATIO; r++) {
                        stereo[stereo_idx++] = s_vol; // L
                        stereo[stereo_idx++] = s_vol; // R
                    }
                }

                if (ringbuf_wav != NULL) {
                    xRingbufferSend(
                        ringbuf_wav,
                        (void*)stereo,
                        stereo_idx * sizeof(int16_t),
                        portMAX_DELAY
                    );
                }

                // Buka "keran" setelah buffer cukup terisi
                prefill_count++;
                if (prefill_count >= 8) {
                    is_wav_playing = true;
                }
            }
            file.close();
        } else {
            Serial.printf("[WAV] Gagal buka: %s\n", wav_to_play);
        }
        wav_to_play[0] = '\0';
    }

    // Beri waktu master task menguras sisa buffer
    vTaskDelay(pdMS_TO_TICKS(500));
    is_wav_playing = false;
    vTaskDelete(NULL);
}

void play_wav(const char *filename) {
    if (is_wav_playing) return; // cegah double play

    strncpy(wav_to_play, filename, sizeof(wav_to_play));
    xTaskCreatePinnedToCore(
        wav_task,
        "wav_task",
        8192,
        NULL,
        4,
        NULL,
        1
    );
}

// ── SECTION 10: DISPLAY FUNCTIONS ───────────────────────────────

// Helper: format ms ke MM:SS
void format_time(char *buf, uint32_t ms) {
    uint32_t total_sec = ms / 1000;
    uint32_t min       = total_sec / 60;
    uint32_t sec       = total_sec % 60;
    snprintf(buf, 8, "%02d:%02d", min, sec);
}

// Helper: scroll otomatis
int get_scroll_offset(const char *text, int max_width, int &scroll_x) {
    int text_width = u8g2.getStrWidth(text);
    if (text_width <= max_width) return 0;

    unsigned long now = millis();

    if (scroll_pausing) {
        if (now - scroll_pause_start > SCROLL_PAUSE_MS) {
            scroll_pausing = false;
        }
        return scroll_x;
    }

    if (now - last_scroll > SCROLL_SPEED_MS) {
        last_scroll = now;
        scroll_x++;
        if (scroll_x > text_width - max_width + 10) {
            scroll_x         = 0;
            scroll_pausing   = true;
            scroll_pause_start = now;
        }
    }
    return scroll_x;
}

void update_display() {
    u8g2.clearBuffer();
    u8g2.setFont(u8g2_font_6x10_tr);

    // ── PAGE_NOWPLAY ─────────────────────────────────────────────
    if (current_page == PAGE_NOWPLAY) {
        if (!a2dp_sink.is_connected() || !track_info.has_data) {
            // Tengah layar
            u8g2.drawStr(30, 18, "No Music");
        } else {
            // Baris 1 — Judul lagu (scroll)
            u8g2.setClipWindow(0, 0, 127, 12);
            int ox = get_scroll_offset(track_info.title, 128, scroll_x_title);
            u8g2.drawStr(-ox, 10, track_info.title);
            u8g2.setMaxClipWindow();

            // Baris 2 — Artist (scroll)
            u8g2.setClipWindow(0, 12, 127, 22);
            int oa = get_scroll_offset(track_info.artist, 128, scroll_x_artist);
            u8g2.drawStr(-oa, 22, track_info.artist);
            u8g2.setMaxClipWindow();

            // Baris 3 — Waktu + progress bar
            char time_cur[8], time_dur[8];
            format_time(time_cur, track_info.position_ms);
            format_time(time_dur, track_info.duration_ms);

            u8g2.drawStr(0, 32, time_cur);
            u8g2.drawStr(100, 32, time_dur);

            // Progress bar
            const int bar_x = 30, bar_y = 25;
            const int bar_w = 68, bar_h = 6;
            u8g2.drawFrame(bar_x, bar_y, bar_w, bar_h);

            if (track_info.duration_ms > 0) {
                int fill = (int)(
                    (float)track_info.position_ms /
                    track_info.duration_ms *
                    (bar_w - 2)
                );
                if (fill > 0) {
                    u8g2.drawBox(bar_x + 1, bar_y + 1, fill, bar_h - 2);
                }
            }
        }

    // ── PAGE_CONFIG ───────────────────────────────────────────────
    } else if (current_page == PAGE_CONFIG) {
        if (ui_state == IDLE) {
            // Tampilan status normal
            u8g2.drawStr(0, 10, a2dp_sink.is_connected() ? "BT: Connected" : "BT: Waiting...");

            char line2[32];
            snprintf(line2, sizeof(line2), "Freq:%dHz %s",
                     (int)crossover_freq, mono_mode ? "XO" : "ST");
            u8g2.drawStr(0, 22, line2);

            char line3[32];
            snprintf(line3, sizeof(line3), "W:%.1f T:%.1f V:%d%%",
                     gain_woofer, gain_tweeter, (int)(wav_volume * 100));
            u8g2.drawStr(0, 32, line3);

        } else {
            // NAVIGATE / EDIT — tampilan parameter
            const char *param_names[PARAM_COUNT] = {
                "Freq", "Gain W", "Gain T", "WAV Vol", "Mode"
            };

            char param_values[PARAM_COUNT][16];
            snprintf(param_values[PARAM_FREQ],    sizeof(param_values[0]), "%dHz", (int)crossover_freq);
            snprintf(param_values[PARAM_GAIN_W],  sizeof(param_values[1]), "%.1f", gain_woofer);
            snprintf(param_values[PARAM_GAIN_T],  sizeof(param_values[2]), "%.1f", gain_tweeter);
            snprintf(param_values[PARAM_WAV_VOL], sizeof(param_values[3]), "%d%%", (int)(wav_volume * 100));
            snprintf(param_values[PARAM_MODE],    sizeof(param_values[4]), mono_mode ? "XO" : "ST");

            // Scroll virtual — max 3 parameter terlihat
            int start = (selected_param > 1) ? selected_param - 1 : 0;
            if (start + 3 > PARAM_COUNT) start = PARAM_COUNT - 3;

            for (int i = 0; i < 3; i++) {
                int idx = start + i;
                if (idx >= PARAM_COUNT) break;

                char line[32];
                snprintf(line, sizeof(line), "%s: %s",
                         param_names[idx], param_values[idx]);

                bool is_selected = (idx == (int)selected_param);

                if (is_selected && blink_state) {
                    u8g2.setDrawColor(1);
                    u8g2.drawBox(0, i * 11, 128, 11);
                    u8g2.setDrawColor(0);
                    u8g2.drawStr(2, i * 11 + 9, line);
                    u8g2.setDrawColor(1);
                } else {
                    u8g2.drawStr(2, i * 11 + 9, line);
                }
            }
        }

    // ── PAGE_CONTROL ──────────────────────────────────────────────
    } else if (current_page == PAGE_CONTROL) {
        const char *icons[CTRL_COUNT]  = { "<<", "||", ">", ">>" };
        const int   box_x[CTRL_COUNT]  = { 4, 36, 68, 100 };
        const int   box_y = 8;
        const int   box_w = 24;
        const int   box_h = 16;

        for (int i = 0; i < CTRL_COUNT; i++) {
            int text_w = u8g2.getStrWidth(icons[i]);
            int text_x = box_x[i] + (box_w / 2) - (text_w / 2);
            int text_y = box_y + (box_h / 2) + 3;

            if (i == (int)selected_action) {
                u8g2.drawBox(box_x[i], box_y, box_w, box_h);
                u8g2.setDrawColor(0);
                u8g2.drawStr(text_x, text_y, icons[i]);
                u8g2.setDrawColor(1);
            } else {
                u8g2.drawStr(text_x, text_y, icons[i]);
            }
        }
    }

    u8g2.sendBuffer();
}

// ── SECTION 11: SETTINGS (NVS) ───────────────────────────────────
Preferences prefs;

void save_settings() {
    prefs.begin("speaker", false);
    prefs.putBool("mono",    mono_mode);
    prefs.putFloat("freq",   crossover_freq);
    prefs.putFloat("gain_w", gain_woofer);
    prefs.putFloat("gain_t", gain_tweeter);
    prefs.putFloat("wav_vol", wav_volume);
    prefs.end();
    Serial.println("[NVS] Settings tersimpan!");
}

void load_settings() {
    prefs.begin("speaker", true);
    mono_mode      = prefs.getBool("mono",    true);
    crossover_freq = prefs.getFloat("freq",   5000.0f);
    gain_woofer    = prefs.getFloat("gain_w", 1.0f);
    gain_tweeter   = prefs.getFloat("gain_t", 1.0f);
    wav_volume     = prefs.getFloat("wav_vol", 0.7f);
    prefs.end();
    Serial.printf("[NVS] Loaded → freq:%.0f gW:%.2f gT:%.2f vol:%.2f mono:%s\n",
                  crossover_freq, gain_woofer, gain_tweeter, wav_volume,
                  mono_mode ? "on" : "off");
}

// ── SECTION 12: ROTARY ENCODER ───────────────────────────────────
int           last_clk    = HIGH;
bool          sw_last     = HIGH;
unsigned long sw_debounce = 0;

void handle_encoder() {
    // ── Rotasi ──
    int clk_val = digitalRead(ENC_CLK);
    if (clk_val != last_clk && clk_val == LOW) {
        last_interaction         = millis();
        last_control_interaction = millis();

        int  dt_val = digitalRead(ENC_DT);
        bool cw     = (dt_val != clk_val);

        if (current_page == PAGE_CONTROL) {
            // Pindah pilihan kontrol
            if (cw) {
                selected_action = (ControlAction)((selected_action + 1) % CTRL_COUNT);
            } else {
                selected_action = (ControlAction)((selected_action - 1 + CTRL_COUNT) % CTRL_COUNT);
            }

        } else if (ui_state == IDLE) {
            // Pindah halaman (CONFIG ↔ NOWPLAY)
            if (cw) {
                current_page = (current_page == PAGE_NOWPLAY) ? PAGE_CONFIG : PAGE_NOWPLAY;
            } else {
                current_page = (current_page == PAGE_CONFIG) ? PAGE_NOWPLAY : PAGE_CONFIG;
            }

        } else if (ui_state == NAVIGATE) {
            if (cw) {
                selected_param = (UIParam)((selected_param + 1) % PARAM_COUNT);
            } else {
                selected_param = (UIParam)((selected_param - 1 + PARAM_COUNT) % PARAM_COUNT);
            }

        } else if (ui_state == EDIT) {
            switch (selected_param) {
                case PARAM_FREQ:
                    crossover_freq = constrain(
                        crossover_freq + (cw ? 100 : -100), 100, 20000);
                    update_crossover(crossover_freq);
                    break;
                case PARAM_GAIN_W:
                    gain_woofer = constrain(
                        gain_woofer + (cw ? 0.1f : -0.1f), 0.0f, 2.0f);
                    break;
                case PARAM_GAIN_T:
                    gain_tweeter = constrain(
                        gain_tweeter + (cw ? 0.1f : -0.1f), 0.0f, 2.0f);
                    break;
                case PARAM_WAV_VOL:
                    wav_volume = constrain(
                        wav_volume + (cw ? 0.1f : -0.1f), 0.0f, 1.0f);
                    break;
                case PARAM_MODE:
                    mono_mode = !mono_mode;
                    break;
                default: break;
            }
        }

        update_display();
    }
    last_clk = clk_val;

    // ── Tombol SW ──
    bool sw_val = digitalRead(ENC_SW);
    if (sw_val == LOW && sw_last == HIGH && millis() - sw_debounce > 200) {
        sw_debounce              = millis();
        last_interaction         = millis();
        last_control_interaction = millis();

        if (current_page == PAGE_CONTROL) {
            // Eksekusi media control via AVRCP
            switch (selected_action) {
                case CTRL_PREV:  a2dp_sink.previous(); break;
                case CTRL_PAUSE: a2dp_sink.pause();    break;
                case CTRL_PLAY:  a2dp_sink.play();     break;
                case CTRL_NEXT:  a2dp_sink.next();     break;
                default: break;
            }
            current_page = PAGE_NOWPLAY;

        } else if (current_page == PAGE_NOWPLAY) {
            current_page    = PAGE_CONTROL;
            selected_action = CTRL_PLAY;

        } else if (current_page == PAGE_CONFIG) {
            if (ui_state == IDLE) {
                ui_state = NAVIGATE;
            } else if (ui_state == NAVIGATE) {
                ui_state = EDIT;
            } else if (ui_state == EDIT) {
                save_settings();
                ui_state = NAVIGATE;
            }
        }

        update_display();
    }
    sw_last = sw_val;
}

void handle_timeout() {
    // CONFIG → IDLE timeout
    if (current_page == PAGE_CONFIG &&
        ui_state != IDLE &&
        millis() - last_interaction > TIMEOUT_MS) {
        if (ui_state == EDIT) save_settings();
        ui_state = IDLE;
        update_display();
    }

    // CONTROL → NOWPLAY timeout
    if (current_page == PAGE_CONTROL &&
        millis() - last_control_interaction > CONTROL_TIMEOUT_MS) {
        current_page = PAGE_NOWPLAY;
        update_display();
    }
}

void handle_blink() {
    unsigned long interval = (ui_state == EDIT) ? BLINK_FAST : BLINK_SLOW;
    if (millis() - last_blink > interval) {
        blink_state = !blink_state;
        last_blink  = millis();
        if (ui_state != IDLE) update_display();
    }
}

// ── SECTION 13: BLUETOOTH CALLBACKS ─────────────────────────────
void bt_connection_state_changed(esp_a2d_connection_state_t state, void *ptr) {
    if (state == ESP_A2D_CONNECTION_STATE_CONNECTED) {
        play_wav("/connected.wav");
        Serial.println("[BT] Connected");
    } else if (state == ESP_A2D_CONNECTION_STATE_DISCONNECTED) {
        // Reset track info
        memset(track_info.title,  0, sizeof(track_info.title));
        memset(track_info.artist, 0, sizeof(track_info.artist));
        track_info.duration_ms = 0;
        track_info.position_ms = 0;
        track_info.has_data    = false;


        scroll_x_title  = 0;
        scroll_x_artist = 0;
        play_wav("/disconnected.wav");
        Serial.println("[BT] Disconnected");
    }
    update_display();
}

void avrc_metadata_callback(uint8_t id, const uint8_t *text) {
    switch (id) {
        case ESP_AVRC_MD_ATTR_TITLE:
            strncpy(track_info.title, (const char*)text,
                    sizeof(track_info.title) - 1);
            track_info.has_data = true;
            scroll_x_title = 0;
            break;
        case ESP_AVRC_MD_ATTR_ARTIST:
            strncpy(track_info.artist, (const char*)text,
                    sizeof(track_info.artist) - 1);
            scroll_x_artist = 0;
            break;
        case ESP_AVRC_MD_ATTR_PLAYING_TIME:
            track_info.duration_ms = atoi((const char*)text);
            break;
    }
    update_display();
}

void avrc_play_pos_callback(uint32_t play_pos) {
    track_info.position_ms = play_pos;
    update_display();
}

// ── SECTION 14: AUDIO CALLBACK ───────────────────────────────────
void audio_data_callback(const uint8_t *data, uint32_t len) {
    size_t num_samples = len / 4;

    // Fixed size — tidak ada VLA
    static int16_t i2s_data[1024];

    for (size_t i = 0; i < num_samples; i++) {
        int16_t left  = (data[i * 4 + 1] << 8) | data[i * 4];
        int16_t right = (data[i * 4 + 3] << 8) | data[i * 4 + 2];
        i2s_data[i * 2]     = right;
        i2s_data[i * 2 + 1] = left;
    }

    if (ringbuf_bt != NULL) {
        xRingbufferSend(
            ringbuf_bt,
            (void*)i2s_data,
            num_samples * 4,
            portMAX_DELAY
        );
    }
}

// ── SECTION 15: MASTER AUDIO TASK ────────────────────────────────
size_t fetch_from_ringbuf(RingbufHandle_t rb, int16_t *dest,
                          size_t max_bytes, TickType_t wait_ticks) {
    size_t   total_read = 0;
    uint8_t *ptr        = (uint8_t *)dest;

    while (total_read < max_bytes) {
        size_t bytes_to_read = max_bytes - total_read;
        size_t received_size = 0;

        // Hanya tunggu di tarikan pertama
        TickType_t ticks = (total_read == 0) ? wait_ticks : 0;

        void *data = xRingbufferReceiveUpTo(
            rb, &received_size, ticks, bytes_to_read);

        if (data == NULL) break;

        memcpy(ptr + total_read, data, received_size);
        vRingbufferReturnItem(rb, data);
        total_read += received_size;
    }
    return total_read;
}

void master_audio_task(void *param) {
    const size_t CHUNK_SIZE = 1024; // bytes per loop

    static int16_t bt_buffer[512];   // 1024 bytes
    static int16_t wav_buffer[512];  // 1024 bytes
    static int16_t mixed[512];       // hasil mix
    static int16_t output[1024];     // output ke I2S (stereo)

    while (true) {
        // ── Ambil data dari ringbuffer ──
        size_t bt_bytes  = fetch_from_ringbuf(
            ringbuf_bt, bt_buffer, CHUNK_SIZE, pdMS_TO_TICKS(10));
        size_t wav_bytes = 0;

        if (is_wav_playing) {
            wav_bytes = fetch_from_ringbuf(
                ringbuf_wav, wav_buffer, CHUNK_SIZE, 0);
        }

        // Kalau tidak ada data sama sekali → delay & lanjut
        if (bt_bytes == 0 && wav_bytes == 0) {
            vTaskDelay(pdMS_TO_TICKS(2));
            continue;
        }

        // ── Software Mixer ──
        size_t max_bytes  = (bt_bytes > wav_bytes) ? bt_bytes : wav_bytes;
        size_t num_frames = max_bytes / 4; // 1 frame = L + R = 4 bytes

        memset(mixed, 0, sizeof(mixed));

        for (size_t i = 0; i < num_frames * 2; i++) {
            int32_t s_bt  = ((i * 2) < bt_bytes)  ? (int32_t)bt_buffer[i]  : 0;
            int32_t s_wav = ((i * 2) < wav_bytes)  ? (int32_t)wav_buffer[i] : 0;
            int32_t sum   = s_bt + s_wav;
            mixed[i]      = (int16_t)constrain(sum, -32768, 32767);
        }

        // ── DSP / Passthrough ──
        if (mono_mode) {
            apply_crossover(mixed, output, num_frames);
        } else {
            // Stereo passthrough — copy langsung
            memcpy(output, mixed, num_frames * 4);
        }

        // ── Output ke DAC via I2S ──
        size_t bytes_written;
        i2s_write(I2S_NUM_0, output, num_frames * 4,
                  &bytes_written, portMAX_DELAY);
    }
}

// ── SECTION 16: SERIAL CONTROLLER ───────────────────────────────
void handle_serial() {
    if (!Serial.available()) return;

    String cmd = Serial.readStringUntil('\n');
    cmd.trim();

    if (cmd.startsWith("freq:")) {
        float freq = cmd.substring(5).toFloat();
        if (freq > 100 && freq < 20000) {
            crossover_freq = freq;
            update_crossover(crossover_freq);
            update_display();
        } else {
            Serial.println("[ERR] Freq harus antara 100 - 20000 Hz");
        }

    } else if (cmd.startsWith("gain_w:")) {
        gain_woofer = constrain(cmd.substring(7).toFloat(), 0.0f, 2.0f);
        Serial.printf("[OK] Gain woofer: %.2f\n", gain_woofer);
        update_display();

    } else if (cmd.startsWith("gain_t:")) {
        gain_tweeter = constrain(cmd.substring(7).toFloat(), 0.0f, 2.0f);
        Serial.printf("[OK] Gain tweeter: %.2f\n", gain_tweeter);
        update_display();

    } else if (cmd.startsWith("wav_vol:")) {
        float vol = cmd.substring(8).toFloat();
        if (vol >= 0.0f && vol <= 1.0f) {
            wav_volume = vol;
            Serial.printf("[OK] WAV volume: %.0f%%\n", vol * 100);
            update_display();
        } else {
            Serial.println("[ERR] Nilai harus antara 0.0 - 1.0");
        }

    } else if (cmd.startsWith("mono:")) {
        String val = cmd.substring(5);
        if (val == "on") {
            mono_mode = true;
            Serial.println("[OK] Mode: Crossover (XO)");
        } else if (val == "off") {
            mono_mode = false;
            Serial.println("[OK] Mode: Stereo (ST)");
        } else {
            Serial.println("[ERR] Gunakan 'mono:on' atau 'mono:off'");
        }
        update_display();

    } else if (cmd == "save") {
        save_settings();

    } else if (cmd == "reset") {
        mono_mode      = true;
        crossover_freq = 5000.0f;
        gain_woofer    = 1.0f;
        gain_tweeter   = 1.0f;
        wav_volume     = 0.7f;
        update_crossover(crossover_freq);
        save_settings();
        update_display();
        Serial.println("[OK] Reset ke default");

    } else if (cmd == "status") {
        Serial.println("========== Status ==========");
        Serial.printf("Mode     : %s\n",  mono_mode ? "XO" : "ST");
        Serial.printf("Crossover: %.0f Hz\n", crossover_freq);
        Serial.printf("Gain W   : %.2f\n",    gain_woofer);
        Serial.printf("Gain T   : %.2f\n",    gain_tweeter);
        Serial.printf("WAV Vol  : %.0f%%\n",  wav_volume * 100);
        Serial.printf("BT       : %s\n",
            a2dp_sink.is_connected() ? "Connected" : "Waiting");
        Serial.printf("Free heap: %d bytes\n", ESP.getFreeHeap());
        Serial.println("============================");

    } else if (cmd == "help") {
        Serial.println("======= Serial Controller =======");
        Serial.println("freq:<Hz>     → crossover (100-20000)");
        Serial.println("gain_w:<val>  → gain woofer (0.0-2.0)");
        Serial.println("gain_t:<val>  → gain tweeter (0.0-2.0)");
        Serial.println("wav_vol:<val> → volume WAV (0.0-1.0)");
        Serial.println("mono:on/off   → mode XO / stereo");
        Serial.println("save          → simpan settings");
        Serial.println("reset         → default settings");
        Serial.println("status        → lihat status");
        Serial.println("help          → daftar perintah");
        Serial.println("=================================");

    } else {
        Serial.printf("[ERR] Perintah tidak dikenal: %s\n", cmd.c_str());
        Serial.println("Ketik 'help' untuk bantuan");
    }
}

// ── SECTION 17: SETUP & LOOP ─────────────────────────────────────
void setup() {
    Serial.begin(115200);
    Serial.println("\n[BOOT] ESP32 Bluetooth Speaker");

    // ── GPIO ──
    pinMode(BTLED,   OUTPUT);
    pinMode(ENC_CLK, INPUT_PULLUP);
    pinMode(ENC_DT,  INPUT_PULLUP);
    pinMode(ENC_SW,  INPUT_PULLUP);

    // ── OLED ──
    u8g2.begin();
    update_display();

    // ── Settings ──
    load_settings();
    update_crossover(crossover_freq);
    Serial.println("[OK] Settings loaded");

    // ── LittleFS ──
    if (!LittleFS.begin(true)) {
        Serial.println("[ERR] LittleFS Mount Failed!");
        return;
    }
    Serial.println("[OK] LittleFS mounted");

    // ── I2S ──
    i2s_driver_install(I2S_NUM_0, &i2s_config, 0, NULL);
    i2s_set_pin(I2S_NUM_0, &pin_config);
    Serial.println("[OK] I2S ready");

    // ── Ring Buffer ──
    init_ring_buffers();

    // ── Master Audio Task → Core 1, prioritas tertinggi ──
    xTaskCreatePinnedToCore(
        master_audio_task,
        "MasterAudio",
        8192,
        NULL,
        configMAX_PRIORITIES - 1,
        NULL,
        1
    );
    Serial.println("[OK] Master audio task started");

    // ── Bluetooth A2DP ──
    a2dp_sink.set_on_connection_state_changed(bt_connection_state_changed);
    a2dp_sink.set_stream_reader(audio_data_callback, false);
    a2dp_sink.set_auto_reconnect(true);
    a2dp_sink.set_avrc_metadata_attribute_mask(
        ESP_AVRC_MD_ATTR_TITLE        |
        ESP_AVRC_MD_ATTR_ARTIST       |
        ESP_AVRC_MD_ATTR_PLAYING_TIME
    );
    a2dp_sink.set_avrc_metadata_callback(avrc_metadata_callback);
    a2dp_sink.set_avrc_rn_play_pos_callback(avrc_play_pos_callback, 1);
    a2dp_sink.start("Speaker Mahal 😁");
    Serial.println("[OK] Bluetooth started");

    Serial.println("[READY] Ketik 'help' untuk daftar perintah");
    update_display();
}

void loop() {
    digitalWrite(BTLED, a2dp_sink.is_connected() ? HIGH : LOW);
    handle_serial();
    handle_encoder();
    handle_timeout();
    handle_blink();
}